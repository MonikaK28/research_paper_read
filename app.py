...
def extract_pages(uploaded_file):
    ...
    return [{"page_num": i+1, 
    "text": page.get_text()} for i, page in enumerate(doc)]
...
def build_agent(pages):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a medical research assistant. Search PDF first; fall back to web if NO_CONTENT_FOUND. Use bullet points and tables. Current year: 2026."),
        ("human", "{input}"),
        ("placeholder", "{agent_scratchpad}"),
    ])
    return AgentExecutor(agent=create_tool_calling_agent(llm, [search_pdf, web_search], prompt), tools=[search_pdf, web_search], verbose=True), len(chunks)
